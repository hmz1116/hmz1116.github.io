...
var MAX_STEP = 8;
var PIECE_NAME = [
  "oo", null, null, null, null, null, null, null, null, null, null, null, null, null, null, null,
  "rk", "ra", "rb", "rn", "rr", "rc", "rp", "rs", "rd", "rv", "rx", "rf", "rw", null, null, null,
  "bk", "ba", "bb", "bn", "br", "bc", "bp", "bs", "bd", "bv", "bx", "bf", "bw", null, null, null,
];
...
Board.prototype.addMove = function(mv, computerMove) {
  this.pos.lastmovepc[this.pos.sdPlayer] = this.pos.squares[SRC(mv)] & PCNOMAX;
...
Board.prototype.clickSquare = function(sq_) {
  ...
  if ((pc & SIDE_TAG(this.pos.sdPlayer)) != 0 && !this.phantomready[sd]) {
    ...
    if(this.sqSelected == sq){
      if((pc & 15) == 0 && this.pos.bDragon(sd)){
        this.dragonready[sd] = !this.dragonready[sd];
      }
      else if ((pc & 15) == 11) {
    ...
    if(this.dragonready[sd] && (this.pos.squares[this.sqSelected] & 15) == 0){
      var sqdg = 0;
...
Board.prototype.retract = function() {
  ...
  if(this.pos.mvList.length > 0){
    var mv = this.pos.mvList[this.pos.mvList.length-1];
    var sqSrc = SRC(mv);
    var sqDst = DST(mv);
    if(sqDst < 0x10)sqDst = DG(sqSrc,sqDst);
    this.pos.lastmovepc[1-this.pos.sdPlayer] = this.pos.squares[sqDst] & PCNOMAX;
    if(this.pos.mvList.length > 1){
      mv = this.pos.mvList[this.pos.mvList.length-2];
      sqSrc = SRC(mv);
      sqDst = DST(mv);
      if(sqDst < 0x10)sqDst = DG(sqSrc,sqDst);
      this.pos.lastmovepc[this.pos.sdPlayer] = this.pos.squares[sqDst] & PCNOMAX;
    }
    else{
      this.pos.lastmovepc[this.pos.sdPlayer] = PIECE_WIZARD;
    }
